...
docker build -t $DOCKER_USERNAME/app:$DEPLOY_TAG .
docker push $DOCKER_USERNAME/app:$DEPLOY_TAG
...
ssh -o "StrictHostKeyChecking=no" $GCP_VM_SSH_URL << EOF
  docker pull $DOCKER_USERNAME/app:$DEPLOY_TAG
  docker stop staging-app || true
  docker rm staging-app || true
  docker run -d --name staging-app -p 80:3000 $DOCKER_USERNAME/app:$DEPLOY_TAG
